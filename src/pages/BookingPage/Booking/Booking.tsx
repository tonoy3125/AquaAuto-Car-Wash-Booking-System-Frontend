...
const Booking = () => {
  ...
  const uniqueServices = userBookings?.data?.reduce((acc, booking) => {
    const serviceName = booking?.service?.name;
    if (
      serviceName &&
      !acc.some((item) => item?.service?.name === serviceName)
    ) {
      acc.push(booking);
    }
    return acc;
      ...
      {isLoading ? ( // Replace `loading` with your actual loading state
        ...
        <div className="flex items-start flex-col lg:flex-row gap-5 ">
          ...
          <div className="w-[290px] sm:w-[350px] semi-sm:w-[400px] md:w-[730px] lg:w-[350px] bg-[#FFFFFF] shadow-lg rounded-[5px] pb-5 mt-12 mx-auto">
            <ul>
              {uniqueServices?.map((booking, index) => (
                <li
                  key={index}
                  ...
                  onMouseEnter={() => setHoveredService(booking?.service?.name)}
